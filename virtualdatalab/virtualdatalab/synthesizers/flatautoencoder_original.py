...
def _loss_function(input_data, target_data, mu, logvar, max_cat_idx):
    ...
    if max_cat_idx is not None:
        cat_loss = multilabel_soft_margin_loss(input_data[:, :max_cat_idx], target_data[:, :max_cat_idx],reduction='mean')
        num_loss = mse_loss(input_data[:, max_cat_idx:], target_data[:, max_cat_idx:],reduction='mean')
...
class OriginalFlatAutoEncoderSynthesizer(BaseSynthesizer):
    """
